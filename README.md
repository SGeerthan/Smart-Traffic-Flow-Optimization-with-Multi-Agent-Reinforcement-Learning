🚦 Smart Traffic Flow Optimization with Multi-Agent Reinforcement Learning

📌 Description

This project implements an intelligent traffic management dashboard using Multi-Agent Reinforcement Learning (MARL) to optimize traffic signal control in real time. Each traffic intersection is modeled as an autonomous learning agent that dynamically adjusts signal phases based on local traffic conditions, with the goal of minimizing congestion and improving overall traffic efficiency.

The system is trained and evaluated using the SUMO traffic simulator and visualized through an interactive dashboard that provides real-time insights into traffic flow, agent decisions, and performance metrics.
